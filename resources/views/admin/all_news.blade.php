...
<div class="container">
	<a class="btn btn-primary" href="{{url('admin/home/create')}}"><i class="fa fa-paint-brush"> </i> Create</a><br><br>
		...
		<table class="table">
			...
			<tbody>
				<tr>
					<td><i class="fa fa-comments"> </i>  {{$row->n_title}}</td>
					<!-- <td>{{$row->n_content}}</td> -->
					<!-- <td><img class="img-responsive" src="{{asset($row->n_img)}}" width="50" height="50"></td> -->
				 	<td>{{$row->created_at}}</td>
					<!-- <td>{{$row->updated_at}}</td> -->
					<td>
						<a class="fl btn btn-primary" id="edit" href="{{url('admin/home/'.$row->n_id.'/edit') }}">EDIT</a>
						<form action="{{url('admin/home/'.$row->n_id) }}" method="post" onsubmit="return(confirm('Do you want to delete ?'))">
						{{ method_field('DELETE') }}
						{{ csrf_field() }}
